...
const Review = () => {
    ...
    const prevSlider = () => setCurrentSlider((currentSlider) => (currentSlider === 0 ? userreviews.length - 2 : currentSlider - 1));
    const nextSlider = () => setCurrentSlider((currentSlider) => (currentSlider === userreviews.length - 2 ? 0 : currentSlider + 1));
    ...
    return (
        <div className="hero min-h-screen my-6" style={{ backgroundImage: 'url(https://i.ibb.co/9yFBh09/Bright-Modern-Eat-Well-Feel-Well-Instagram-Post.png)' }}>
                        ...
                        {userreviews.map((each, idx) => (
                            <div key={idx} className="p-4 min-w-full md:min-w-[50%] ">
                                <div className="h-full p-8 rounded shadow-[0px_4px_12px_rgba(0,0,0,0.1)] bg-white">
                                    ...
                                    <p className="leading-relaxed mb-6 text-gray-500"> {each?.meal_review}</p>
                                    <p className="text-xl text-black">Item: {each?.meal_title}</p>
                                    <a className="inline-flex items-center">

                                        <img alt="testimonial" src={`https://i.ibb.co/Jz31038/Aesthetic-Elegant-Mindful-Habits-Carousel-Social-Media.png`} className="w-12 h-12 rounded-full flex-shrink-0 object-cover object-center" />
                                        <span className="flex-grow flex flex-col pl-4">
                                            <span className="title-font font-medium text-gray-900">{each.review_username}</span>
